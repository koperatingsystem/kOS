...
| 4 | Next sector | `uint64_t` | Must be smaller than or equal to the number of sectors available for data in total. Must be bigger than the previous sector unless it's 0. | Set to 0 if `kFS_File_Storage_Header_Attributes_Is_Last_Sector` is set. |
| 12 | Previous sector | `uint64_t` | Must be smaller than the number of sectors available for data in total. Must be smaller than the next sector unless it's 0. | Set to 0 if `kFS_File_Storage_Header_Attributes_Is_First_Sector` is set. |
